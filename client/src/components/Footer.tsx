...
const Footer = () => {
  ...
  return (
    <footer className="relative border-t border-mystical-800/30 py-12">
      ...
      <div className="fixed inset-0 bg-cosmic-950/80 -z-10"></div>
      
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
          <h3 className="text-2xl font-cinzel font-bold text-yellow-600 mb-4">
            STARGATE HEALING
          </h3>
          ...
          <div className="flex flex-wrap justify-center gap-6 mb-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-yellow-600 hover:text-yellow-500 transition-colors duration-300 text-sm"
              >
                {item.name}
              </a>
            ))}
